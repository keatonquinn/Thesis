...
\begin{center} {\sf DRAFT VERSION 3 --- {\today} } \end{center}

\begin{abstract}
In this work we study the asymptotic behavior of certain foliations of ends of quasi-Fuchsian manifolds. 
These foliations we call asymptotically Poincar\'e familes as they are asymptotic to the Poincar\'e family as the surfaces leave the end of the manifold.
We prove the limiting behavior of an asymptotically Poincar\'e family ties together the geometry of the quasi-Fuchsian manifold $M$ with the conformal structure on the ideal boundary of (the end of ) $M$. 
...
At $\epsilon  = 0$, the tangent vectors $\dot{[I_\epsilon]} = c \mathrm{Re}(\phi)$ and $\dot{[\two_\epsilon]} = 0$ where $\phi$ is the holomorphic quadratic differential that parametrizes the complex projective structure on the conformal boundary and $c$ a positive constant. 
...
We show the constant mean curvature foliation of Mazzeo and Pacard \cite{mazzeo-pacard2011} forms an asymptotically Poincar\'e family and so we can describe their asymptotics as well. 
\end{abstract}
...
These geometric data take the form of conformal metrics on $\partial^\infty \H^3 = \CP^1$, and given such a metric $\sigma$ on a domain $\Omega \subset \CP^1$ he constructs a map $\mathrm{Ep}_\sigma: \Omega \to \H^3$ whose image is called an Epstein surface.
This Epstein map enjoys an equivariance property with respect to the action of $\mathrm{PSL}_2\C$ which allows one to define Epstein surfaces on certain quotients by subgroups $\Gamma < \mathrm{PSL}_2\C$.
We are concerned with Epstein surfaces in quasi-Fuchsian manifolds: 3-manifolds that for now we think of as quotients $\H^3/\Gamma$ of hyperbolic space by quasi-Fuchsian groups. 

A quasi-Fuchsian manifold has two ends, each diffeomorphic to $S \times [0,1)$ for $S$ a closed surface of genus at least 2.
Each end is compactified by a copy of $S$, which is called the surface at infinity (for the end), and this copy inherits a complex projective structure $Z$ and hence a conformal structure $X$. 
The Epstein construction takes a conformal metric $\sigma$ on $X$ and returns a surface in the quasi-Fuchsian manifold.
When $h$ is the unique hyperbolic metric on $X$, the corresponding Epstein surface we called the Poincar\'e surface of $X$. 
...
The Poincar\'e surface with its parallel copies form a family we call the Poincar\'e family and this collection of surfaces forms a foliation of the end of the quasi-Fuchsian manifold. 

The Poincar\'e family has been used by many to study the geometry of projective surfaces and hyperbolic 3-manifolds. 
...
We study in this thesis families of Epstein surfaces for conformal metrics $\sigma(\epsilon)$ such that there is a scaling function $f$ which satisfies $f(\epsilon)\sigma(\epsilon) \to h$ as $\epsilon \to 0$ (see Definition \ref{asym-def} in Section \ref{asym-def-section} for the precise conditions). 
These families are, in a sense, $C^\infty$-asymptotic to the Poincar\'e family and so we name the \emph{Asymptotically Poincar\'e Families}. 
...
Since Epstein surfaces are strictly convex, the second fundamental form $\two_\epsilon$ is positive definite and so a Riemannian metric, giving another point $[\two_\epsilon]$ in $\mathcal{T}(S)$.
...
In a quasi-Fuchsian manifold, let $S_\epsilon$, for $\epsilon \in (0,1)$, be an asymptotically Poincar\'e family of surfaces with surface at infinity $X$.
...
Our Theorem \ref{main-thm-intro} applies to the work of Mazzeo and Pacard in the setting of constant mean curvature surfaces.
In \cite{mazzeo-pacard2011} they show that the ends of a quasi-Fuchsian manifold admits a unique foliation by surfaces of constant mean curvature. 
...
Chapter \ref{main-results-chapter} defines our asymptotically Poincar\'e surfaces and gives their main results, including Theorem \ref{main-thm-intro}.
...
In our setting the normal bundle is a line bundle and a section is called a normal vector field on $S$. 
For the rest of this section we assume there is a unit normal vector field $n$ on $S$. 
...
This set-theoretic model can be improved to a model of Teichm\"uller space as a quotient manifold. To do so it is necessary to impose add some regularity hypothesis on the space of symmetric 2-tensors that makes them into a Hilbert space. While this is done in Section \ref{sobolev} (following \cite{tromba1992}) here we will simply assume this regularity has been imposed.  
...
\]
(this metric is related to the Weil-Petersson metric on the quotient, but we will not use this, see \cite{tromba1992}). By our regularity assumptions, for each $g$ the inner product $\left< \cdot, \cdot \right>_g$ turns $T_g\mathrm{Met}(S)$ into a Hilbert space. 
...
\[
T_{[g]} \mathcal{T}(S) = T_g \mathrm{Met}(S)/T_g(\mathrm{Diff}_0(S) \times P(S) \cdot g) \cong \{ \dot{g} \ | \ \mathrm{tr}_g(\dot{g}) = 0 = \mathrm{div}_g(\dot{g})\},
\]
...
Following \cite{dumas2009}, we define the difference of the two projective structures as this measure of compatability
\[
Z - W := S(Id).
\]
...
Since $\Omega$ is an open subset of $\CP^1$, it is a complex manifold. 
...
Given two Riemann surfaces $X$ and $Y$ diffeomorphic to $S$, there exists an isomorphism $\rho: \pi_1(S) \to \Gamma < \mathrm{PSL}_2\C$ such that $\H^3/\Gamma$ is a quasi-Fuchsian manifold with surfaces at infinity $\Omega_+/\Gamma = X$ and $\Omega_- / \Gamma = \bar{Y}$.
Here $\bar{Y}$ is the complex conjugate of $Y$ and it has underlying oriented smooth surface is $\bar{S}$, the $S$ with the opposite orientation.
...
\]
This equation is independent of the frame and has in intrinsic meaning, namely the ratio of the volume forms, which is given by taking square roots. 
...
That is, we use 
\[
\mathcal{T}(X) = \mathrm{Met}^\infty(X)/  \mathrm{Diff}_0^\infty(X) \ltimes P^\infty(X),
\]
...
Preliminary calculations show that yes, $[I(\sigma(\epsilon))]$ and $[\two(\sigma(\epsilon))]$ converge to $[\sigma_0] = [h]$.
Moreover, we should still have $\dot{[\two(\sigma(\epsilon))]} = 0$.
...
It would be nice to find a more explicit description of the holomorphic part of a quadratic differential, and to make these computations precise.  
...
\begin{align*}
\widetilde{\mathrm{Ep}}_\sigma(x) = \def\arraystretch{1.5}\begin{pmatrix}
Id_{n-1} & -x & x \\
...
We hope to use this description of the Epstein map to get closed formulas for the induced metric on the Epstein surface, perhaps in terms of the Schwarzian tensor of Osgood and Stowe \cite{osgood-stowe1992}.
